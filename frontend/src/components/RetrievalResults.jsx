  ...
  return (
    <div className="mt-4 mb-2 animate-fade-in pl-12 sm:pl-16">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-xs font-bold uppercase tracking-wider text-accent/80 font-sans m-0">
        ...
        {results.map((c, idx) => (
          ...
          >
            ...
            <div className="w-full flex justify-between items-start gap-3 mb-3">
              ...
              {c.score != null && (
                <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-surface-hover border border-border text-[10px] font-bold text-content-secondary group-hover:text-accent group-hover:border-accent/30 transition-colors" title={`Relevance Score: ${c.score.toFixed(2)}`}>
            ...
            <div className="mt-auto w-full pt-3 border-t border-border/40 flex justify-between items-center text-xs text-content-muted">
              <span className="font-mono bg-surface-hover/50 px-2 py-0.5 rounded text-[10px]">{c.decision_date || 'Unknown Date'}</span>
