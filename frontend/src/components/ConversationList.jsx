  ...
  return (
    <>
      ...
      <div className="p-3 border-t border-sidebar-border/50 bg-sidebar-bg relative">
        ...
        >
          <div className="w-8 h-8 rounded-full accent-gradient-bg flex items-center justify-center text-white ring-1 ring-accent/20 group-hover:ring-accent/50 group-hover:shadow-glow-sm transition-all shrink-0">
            <span className="text-xs font-bold">{initials}</span>
          </div>
          <div className="flex-1 min-w-0 text-left">
            <p className="text-sm font-medium text-sidebar-text/90 truncate group-hover:text-sidebar-text">{user.username}</p>
            <p className="text-[10px] text-sidebar-muted truncate capitalize">{user.role}</p>
          </div>
          <svg
            className={`w-4 h-4 text-sidebar-muted/60 shrink-0 transition-transform duration-200 ${menuOpen ? 'rotate-180' : ''}`}
        ...
        {menuOpen && (
          <div className="absolute bottom-full left-3 right-3 mb-1 bg-sidebar-active border border-sidebar-border/70 rounded-xl shadow-xl overflow-hidden z-50">
            <button
              type="button"
              onClick={() => { setMenuOpen(false); setShowChangePassword(true); }}
              className="w-full flex items-center gap-3 px-4 py-3 text-sm text-sidebar-muted hover:text-sidebar-text hover:bg-sidebar-hover transition-colors"
            >
    ...
    <div className="flex flex-col h-full bg-sidebar-bg text-sidebar-text">
      ...
      <div className="flex items-center gap-3 px-5 py-6 border-b border-sidebar-border/50">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-sidebar-text text-3xl font-serif font-bold tracking-tight">InteLex</h1>
        </div>
        {/* Theme toggle */}
        <button
          type="button"
          ...
          className="shrink-0 w-8 h-8 flex items-center justify-center rounded-lg text-sidebar-muted hover:text-sidebar-text hover:bg-sidebar-active/60 transition-colors"
        >
          {isDark ? (
            /* Sun icon */
      ...
      <div className="p-4 px-3 space-y-2">
        <button
          onClick={onCreate}
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 accent-gradient-bg hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed text-white py-3 px-4 rounded-xl transition-all duration-200 shadow-glow-sm hover:shadow-glow font-medium group"
        >
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-2 pb-2 scrollbar-hide">
        {loading && conversations.length === 0 ? (
          <div className="text-center py-8 text-sidebar-muted text-sm px-4">
          ...
          <div className="space-y-1">
            <h3 className="px-3 py-2 text-[10px] font-bold text-sidebar-muted/70 uppercase tracking-wider mb-1 mt-2">Recents</h3>
            {conversations.map((conv) => {
              ...
              return (
                ...
                >
                  ...
                  {isConfirming && (
                    <div className="absolute inset-0 z-10 rounded-lg bg-sidebar-active border border-red-500/30 flex items-center justify-between px-3 gap-2">
                      <span className="text-xs text-sidebar-text/90 truncate">Delete this chat?</span>
                      <div className="flex gap-1.5 shrink-0">
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 rounded-md bg-sidebar-hover hover:bg-sidebar-active text-sidebar-text/70 hover:text-sidebar-text text-xs font-medium transition-colors"
                        >
                          Cancel
                        </button>
                      </div>
                  ...
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-lg pr-8 ${
                      isActive ? 'text-sidebar-text' : 'text-sidebar-muted hover:text-sidebar-text'
                    }`}
                  >
                    <div className="font-medium truncate text-sm w-full flex items-center gap-2">
                      <svg
                        className={`w-3.5 h-3.5 flex-shrink-0 ${isActive ? 'text-accent' : 'text-sidebar-muted/50'}`}
                        fill="none" viewBox="0 0 24 24" stroke="currentColor"
                      >
                    ...
                    <div className={`text-[10px] truncate mt-1.5 pl-5.5 font-medium ${isActive ? 'text-accent/70' : 'text-sidebar-muted/60'}`}>
                      {formatDate(conv.updated_at || conv.created_at)}
                    </div>
                    {isActive && (
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 accent-gradient-bg rounded-r-full shadow-glow-sm" />
                    )}
                  </button>
                  ...
                  {(isHovered || isActive) && !isConfirming && (
                    <button
                      ...
                      className="absolute right-2 top-1/2 -translate-y-1/2 w-6 h-6 flex items-center justify-center rounded-md text-sidebar-muted/50 hover:text-red-400 hover:bg-red-500/10 transition-colors z-10"
                    >
